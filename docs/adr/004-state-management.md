...
isola needs to track:
- Port assignments per branch/service
- Running process PIDs
- Service status (running/stopped)
- Proxy configuration

Multiple isola processes might access state simultaneously:
- User runs `isola up` in one terminal
- User runs `isola ls` in another terminal
