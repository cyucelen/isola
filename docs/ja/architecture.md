...
isola は Git Worktree Server Manager で、複数の git worktree にまたがる開発サーバーを、自動ポート割り当てとサブドメインベースのルーティングで管理します。
...
│  (isola up/down/ls)  │  (isola dash)     │  (*.localhost)    │
└───────────┬────────────┴────────┬───────────┴────────┬──────────┘
            │                     │                    │
            ▼                     ▼                    ▼
┌───────────────────────────────────────────────────────────────────┐
...
### 1. 設定の読み込み
```
.isola.toml → config.Load() → Config{Services, ProxyPorts}
```
