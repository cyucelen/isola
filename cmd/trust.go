...
Run 'isola proxy start --https' first to generate the CA certificate.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		caPath := filepath.Join(stateRoot, ".isola", "certs", "ca.crt")
		if _, err := os.Stat(caPath); os.IsNotExist(err) {
			return fmt.Errorf("CA certificate not found at %s\nRun 'isola proxy start --https' first to generate certificates", caPath)
		}
...
func trustLinux(caPath string) error {
	destDir := "/usr/local/share/ca-certificates/isola"
	destPath := filepath.Join(destDir, "isola-dev-ca.crt")
